...
% SNR = 0:5:25;
% snr = 10.^(SNR/10);
Nmse_nHK = zeros(1,length(SNR));
Nmse_nHK1= zeros(1,length(SNR));
Nmse_nHK14 = zeros(1,length(SNR));
Nmse_nHK3 = zeros(1,length(SNR));
Nmse_nHK4 = zeros(1,length(SNR));
Nmse_nHK7 = zeros(1,length(SNR));
Nmse_nHK22 = zeros(1,length(SNR));

V_crb = zeros(1,length(SNR));
% max_e1 = 1;%   
    ...
    for s = 1:length(LL1)
       ...
       %% OMP
        ...
        MAX_y_k = Location(K-K_p+1:K);
            ...
            [est_g,I] = My_est_g1(y_w,MAX_y_k,GG,K_p,MSE,max_e1,K,M,L_r,R_W, L_t); %%  1组
            [est_g16,I16] = My_est_g(y_w,MAX_y_k,GG,K_p,MSE,max_e1,K,G1,M,L_r,L_t,RR_W);%%  16组
            [est_g14,I4] = My_est_g4(y_w,MAX_y_k,GG,K_p,MSE,max_e1,K,G2,M,L_r,L_t,RR_W1);%% 4组
...
%        NMSE_nHK(a,s) = sum(norm_Fro_nHK)/sum(norm_Fro_HK);%OMP分为1组
%        NMSE_nHK7(a,s) = sum(norm_Fro_nHK7)/sum(norm_Fro_HK);%OMP分为16组?
%        NMSE_nHK14(a,s) = sum(norm_Fro_nHK14)/sum(norm_Fro_HK);%OMP分为4组?
%        NMSE_nHK1(a,s) = sum(norm_Fro_nHK1)/sum(norm_Fro_HK);%新方法分为1组
%        NMSE_nHK4(a,s) = sum(norm_Fro_nHK4)/sum(norm_Fro_HK);%新方法分为16组
%        NMSE_nHK22(a,s) = sum(norm_Fro_nHK22)/sum(norm_Fro_HK);%新方法分为4组
%        NMSE_nHK3(a,s) = sum(norm_Fro_nHK3)/sum(norm_Fro_HK);%下界
%    
%        nMSE_nHK(a,s) = 10*log10(NMSE_nHK(a,s));%OMP分为1组
%        nMSE_nHK7(a,s) = 10*log10(NMSE_nHK7(a,s));%OMP分为16组
%        nMSE_nHK14(a,s) = 10*log10(NMSE_nHK14(a,s));%OMP分为4组? 
%        nMSE_nHK1(a,s)= 10*log10(NMSE_nHK1(a,s));%新方法分为1组
%        nMSE_nHK4(a,s) = 10*log10(NMSE_nHK4(a,s));%新方法分为16组
%        nMSE_nHK22(a,s) = 10*log10(NMSE_nHK22(a,s));%新方法分为4组
...
SS_RR(:,7) = S_R161;
figure(1)
plot(LL1,S_R,'g-o','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R1,'g-o','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R4,'g->','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R16,'g-<','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R3,'b-*','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R11,'r-o','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R14,'r->','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;
plot(LL1,S_R161,'r-<','Markersize',7,'Linewidth',1);%SS-SW-OMP+Th，P=1
hold on;

% figure(2)
% plot(LL1,Nmse_nHK,'g-o','Markersize',7,'Linewidth',3);%SS-SW-OMP+Th，P=1
% hold on;
% plot(LL1,Nmse_nHK1,'r-o','Markersize',7,'Linewidth',3);%NewMethod，P=1
% hold on;
% plot(LL1,Nmse_nHK3,'b-*','Markersize',7,'Linewidth',3);%LowerBound
% hold on;
% plot(LL1,Nmse_nHK4,'r->','Markersize',7,'Linewidth',3);%NewMethod，P=16
% hold on;
% plot(LL1,Nmse_nHK7,'g->','Markersize',7,'Linewidth',3);%SS-SW-OMP+Th，P=16
% hold on;
% plot(LL1,Nmse_nHK22,'r-<','Markersize',7,'Linewidth',3);%NewMethod，P=4
% hold on;
% plot(LL1,Nmse_nHK14,'g-<','Markersize',7,'Linewidth',3);%SS-SW-OMP+Th，P=4
% hold on;
% plot(LL1,v,'r-*','Markersize',7,'Linewidth',1);
xlabel('LL1'),ylabel('NMSE[dB]');
% % set(gca,'XLim',[32,96]);
