...
%% ********************************************************OMP
 ...
 %% CRB
        ...
        NMSE_nHK(1,a) = sum(norm_Fro_nHK)/sum(norm_Fro_HK);%OMP分为1�?
        NMSE_nHK7(1,a) = sum(norm_Fro_nHK7)/sum(norm_Fro_HK);%OMP分为16�??
        NMSE_nHK14(1,a) = sum(norm_Fro_nHK14)/sum(norm_Fro_HK);%OMP分为4�??
        NMSE_nHK1(1,a) = sum(norm_Fro_nHK1)/sum(norm_Fro_HK);%新方法分�?1�?
        NMSE_nHK4(1,a) = sum(norm_Fro_nHK4)/sum(norm_Fro_HK);%新方法分�?16�?
        NMSE_nHK22(1,a) = sum(norm_Fro_nHK22)/sum(norm_Fro_HK);%新方法分�?4�?
        NMSE_nHK3(1,a) = sum(norm_Fro_nHK3)/sum(norm_Fro_HK);%下界
          V_CRB(1,a) = Sum_vv_k/sum(norm_Fro_HK);
     
        nMSE_nHK(1,a) = 10*log10(NMSE_nHK(1,a));%OMP分为1�?
        nMSE_nHK7(1,a) = 10*log10(NMSE_nHK7(1,a));%OMP分为16�?
        nMSE_nHK14(1,a) = 10*log10(NMSE_nHK14(1,a));%OMP分为4�?? 
        nMSE_nHK1(1,a) = 10*log10(NMSE_nHK1(1,a));%新方法分�?1�?
        nMSE_nHK4(1,a) = 10*log10(NMSE_nHK4(1,a));%新方法分�?16�?
        nMSE_nHK22(1,a) = 10*log10(NMSE_nHK22(1,a));%新方法分�?4�?
        nMSE_nHK3(1,a) = 10*log10(NMSE_nHK3(1,a));%下界
          v_crb(1,a) = 10*log10(V_CRB(1,a));
    end
    Nmse_nHK(1,s) = sum(nMSE_nHK)/CC;%OMP分为1�?
    Nmse_nHK7(1,s) = sum(nMSE_nHK7)/CC;%OMP分为16�?
    Nmse_nHK14(1,s) = sum(nMSE_nHK14)/CC;%OMP分为4�??
    Nmse_nHK1(1,s) = sum(nMSE_nHK1)/CC;%新方法分�?1�?
    Nmse_nHK4(1,s) = sum(nMSE_nHK4)/CC;%新方法分�?16�?
    Nmse_nHK22(1,s) = sum(nMSE_nHK22)/CC;%新方法分�?4�?
